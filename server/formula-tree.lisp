...
(defun is-bool (n)
  (or (string= "T" (string n)) (string= "NIL" (string n))))
(defun parse-infix (ast)
  (let ((current (car ast)))
	...
	(if (string= current "T")
	    "1"
	    "0")
	(if (not-empty-listp current)
	    (parse-infix current)
	    (progn (setf current (string current))
		   (if (or (string= current ">")
			   (string= current "=")
			   (string= current "&")
			   (string= current "%")
			   (string= current "|"))
		       (concatenate 'string "(" (parse-infix (cdr ast)) " " current " " (parse-infix (cddr ast)) ")")
		       (if (string= current "~")
